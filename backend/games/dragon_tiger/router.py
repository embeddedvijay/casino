...
@router.post("/bet")
async def place_bet(req: DragonTigerBetRequest,credentials=Depends(user_security)):
    bind_request_identity(req,credentials)
    return await manager.place_bet(req)


@router.post("/clear")
async def clear_bets(req: DragonTigerClearRequest,credentials=Depends(user_security)):
    bind_request_identity(req,credentials)
