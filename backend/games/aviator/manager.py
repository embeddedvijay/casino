...
def generate_bot_bets():
    ...
    for _ in range(total):
        amount = random.choice([1000, 20000, 1300, 3500, 4000, 500, 100, 1200, 1500, 2000, 7500, 8700, 12000,
                                1050, 22000, 150, 5500, 8000, 2500, 2100, 3200, 4500, 6000, 9500, 700, 2000])
...
def public_data():
    ...
    for bet in bets_by_round.get(round_id, []):
        all_bets.insert(0, {
            ...
            "user": "You",
            "bet": bet["amount"],
            "cashout": bet.get("cashout_amount", 0.0),
        })
...
async def place_bet(req):
    ...
    old = next(
        (
            bet for bet in round_bets
            if bet["user_id"] == req.user_id and bet["seat"] == req.seat
        ),
        None,
    )
...
async def cashout(req):
    ...
    bet = next(
        (
            item for item in round_bets
            if item["user_id"] == req.user_id
            and item["seat"] == req.seat
    ...
    return {
        "success": True,
        "message": f"Cashout {multiplier}x",
